...
import { ArrowUpRight, CheckCircle2, CreditCard, Package, Sparkles, XCircle } from 'lucide-react'
import { auth } from '@/lib/auth/config'
import { prisma } from '@/lib/db/client'
import { PLAN_FEATURES, PLAN_MARKETING, PLAN_ORDER, PLAN_PRICES_CENTS, getPlanUpgrade } from '@/lib/billing/plans'
import { createBillingPortalSessionAction, createCheckoutSessionAction } from '@/app/actions/billing'
...
const FEATURE_LABELS: { label: string; key: keyof typeof PLAN_FEATURES.BASIC }[] = [
  ...
  { label: 'Beneficios', key: 'hasPromotions' },
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-zinc-500">Gestiona tu suscripcion y las herramientas disponibles para fidelizacion</p>
      </div>
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <h2 className="text-2xl font-black">{PLAN_MARKETING[currentPlan].name}</h2>
              </div>
            </div>
            ...
            <p className="mt-2 max-w-2xl text-sm opacity-75">{PLAN_MARKETING[currentPlan].description}</p>
          </div>
          ...
          <div className="flex flex-col gap-2 sm:items-end">
            ...
            {nextPlan && (
              <form action={createCheckoutSessionAction.bind(null, nextPlan)}>
                <button
                  type="submit"
                  className="flex items-center gap-2 rounded-xl bg-white px-5 py-2.5 text-sm font-semibold shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md"
                >
                  <Sparkles className="h-4 w-4 text-primary-600" />
                  Actualizar a {PLAN_MARKETING[nextPlan].name}
                </button>
              </form>
            )}
          </div>
        ...
        <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm">
          ...
          <div className="flex flex-wrap gap-2">
            {org.addOns.map(addOn => (
              <span key={addOn.addOnType} className="rounded-full border border-primary-200 bg-primary-50 px-3 py-1 text-xs font-semibold text-primary-700">
      ...
      <div className="rounded-2xl border border-zinc-200 bg-white shadow-sm">
        <div className="border-b border-zinc-100 px-6 py-4">
          <h2 className="font-bold text-zinc-900">Caracteristicas de tu plan</h2>
        </div>
              ...
              <div key={key} className="flex items-center justify-between px-6 py-3">
                ...
                <span>
                  {typeof value === 'boolean' ? (
                    ...
                    <span className="text-sm font-semibold text-zinc-900">{maxLabel(value as number)}</span>
                  )}
                </span>
              </div>
            )
      ...
      <div className="rounded-2xl border border-zinc-200 bg-white shadow-sm">
            ...
            return (
              <div
                ...
                className={`rounded-2xl border p-5 ${
                  isCurrent ? 'border-primary-600 bg-primary-50 ring-2 ring-primary-200' : 'border-zinc-200 bg-white'
                }`}
              >
                <div className="mb-2 flex items-center justify-between">
                  <span className="font-black text-zinc-900">{PLAN_MARKETING[plan].name}</span>
                  {isCurrent && (
                    <span className="rounded-full bg-primary-600 px-2 py-0.5 text-xs font-bold text-white">Actual</span>
                  )}
                </div>
                <p className="text-xs leading-5 text-zinc-500">{PLAN_MARKETING[plan].tagline}</p>
                <div className="my-4">
                  <span className="text-2xl font-black text-zinc-900">{euros(PLAN_PRICES_CENTS[plan].monthly)}</span>
                  <span className="text-xs text-zinc-400">/mes</span>
                </div>
                {!isCurrent && (
                  <form action={createCheckoutSessionAction.bind(null, plan)}>
                    <button type="submit" className="w-full rounded-xl bg-primary-600 py-2 text-sm font-semibold text-white transition-all hover:bg-primary-700">
                      {PLAN_ORDER.indexOf(plan) > PLAN_ORDER.indexOf(currentPlan) ? 'Actualizar' : 'Cambiar'}
                    </button>
                  </form>
                )}
              </div>
        ...
        <div className="border-t border-zinc-100 px-6 py-3 text-center">
          <a href="/precios" target="_blank" className="text-sm font-medium text-primary-600 hover:text-primary-700">
