    ...
    <div className="min-h-screen bg-zinc-50">
      ...
      <div className="mx-auto max-w-[640px] px-4 py-12">
        ...
        <div className="mb-8 flex flex-col items-center text-center">
          ...
          <h1 className="text-2xl font-black text-zinc-900">¡Bono adquirido!</h1>
          <p className="mt-2 text-sm text-zinc-500">
            Confirmación enviada a <span className="font-semibold text-zinc-700">{customer.email}</span>
          </p>
          <p className="mt-1 text-xs text-zinc-400">{purchaseDate}</p>
        </div>

        {/* Bono card */}
        <div className="mb-5 overflow-hidden rounded-3xl border border-primary-100 bg-gradient-to-br from-primary-50 to-beauty-50 p-6">
          <div className="mb-3 flex items-center gap-3">
            <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-white shadow-sm">
              <Gift className="h-6 w-6 text-primary-600" />
            </div>
            <div>
              <p className="font-black text-zinc-900">{bono.name}</p>
              <p className="text-sm text-zinc-500">{bono.center.name} · {bono.center.addressCity}</p>
            </div>
          </div>
          ...
          <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
            <div className="rounded-xl bg-white/70 p-3 text-center">
              ...
              <p className="text-lg font-black text-zinc-900">{instance.sessionsRemaining}</p>
              <p className="text-[10px] text-zinc-500">sesiones</p>
            </div>
            <div className="rounded-xl bg-white/70 p-3 text-center">
              <p className="mb-1 text-sm">💶</p>
              <p className="text-lg font-black text-zinc-900">{formatPrice(bono.priceCents)}</p>
              <p className="text-[10px] text-zinc-500">total</p>
            </div>
            {expiryDate && (
              <div className="col-span-2 rounded-xl bg-white/70 p-3 text-center">
                <Calendar className="mx-auto mb-1 h-4 w-4 text-primary-500" />
                <p className="text-sm font-bold text-zinc-900">Válido hasta</p>
                <p className="text-xs text-zinc-500">{expiryDate}</p>
              </div>
            )}
          </div>

          {bono.service && (
            <p className="mt-3 inline-block rounded-full bg-white/80 px-3 py-1 text-xs font-semibold text-primary-700">
        ...
        <div className="mb-5 rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
          <h3 className="mb-1 font-bold text-zinc-900">Referencia del bono</h3>
          <p className="font-mono text-2xl font-black tracking-widest text-primary-600">{shortRef}</p>
          <p className="mt-1 text-xs text-zinc-400">
            Muestra este código al centro para que registren el uso de cada sesión
          </p>
          <p className="mt-2 text-xs text-zinc-300">ID completo: {instance.id}</p>
        </div>
